...
| `deployment/` | Library mode, HTTP mode, React UI, Streamlit UI, Azure deployment, security layers. |
| `providers/` | Built-in provider pages and custom-provider guidance. |
| `configuration/` | `AgentConfig`, env vars, report profiles, [configuration cookbook](configuration/configuration-cookbook.md). |
| `architecture/` | Graph topology, nodes, state, evidence pipeline, public API, result schema, strategies. |
