...
Deploying Inqtrix inside an enterprise Azure tenant: picking an authentication mode, handling Foundry token lifetime, configuring CORS/TLS, and keeping the HTTP server healthy across long-running requests. The Azure-provider details themselves live on the per-provider pages ([Azure OpenAI](../providers/azure-openai.md), [Azure Foundry Web Search](../providers/azure-foundry-web-search.md)).

## Authentication decision tree

This diagram answers: "Which Azure authentication constructor path should I
choose?" It is a deployment decision tree, not runtime agent control flow.

```mermaid
flowchart TD
    A["deployment question: where does Inqtrix run?"] --> B{"router: inside Azure?"}
    B -->|"AKS / App Service / VM with MI"| C["config: DefaultAzureCredential<br/>(Managed Identity)"]
    B -->|"On-prem / other cloud"| D{"Short-lived workload?"}
    D -->|"CI/CD, cron, one-off"| E["config: Service Principal<br/>tenant_id + client_id + client_secret"]
    D -->|"Long-running server"| F{"Have a custom IDP?"}
    F -->|"Yes"| G["config: azure_ad_token_provider"]
    F -->|"No"| E
    B -->|"Single-developer, throwaway"| H["config: api_key"]
```
...
Restarting a container drops active in-flight requests and native-run records,
but ordinary chat continuity is client-side: the client re-sends the visible
history on the next request. Zero-downtime blue-green is possible if you route
traffic so that both colours briefly accept writes; this is the responsibility
of the Azure infrastructure layer and not handled by Inqtrix.
