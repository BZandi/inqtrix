...
|-------|---------|---------------------|----------------|---------------|
| Automated local test suite | `uv run pytest tests/ -v` | No | Unit tests, replay tests, streaming, config, server routes, provider normalisation. Use collect-only for the current count. | Editable install with dev extras. |
