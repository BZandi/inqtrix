...
- `/v1/chat/completions`: compare `MAX_CONCURRENT` (default 3) to active research requests in the access log. If active requests equal the cap, increase the setting. If not, the semaphore may have leaked — the normal cause would be an exception escaping outside the `stream_response` context manager.
