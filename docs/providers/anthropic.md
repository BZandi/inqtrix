...
llm = AnthropicLLM(
    ...
    claim_extract_model="claude-haiku-4-5",  # usually the biggest cost lever
...
| `classify_model` / `evaluate_model` / `claim_extract_model` | Optional per-role overrides. |
| `thinking` | Forwarded on reasoning calls (classify, evaluate, plan, answer). **Not** used for claim extraction. If the model does not support thinking, Anthropic rejects the request with a 400. |
