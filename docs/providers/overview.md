...
All six providers are exported from the top-level `inqtrix` package.

Use the matrix as a menu: import one LLM provider and one search provider, construct them with their own credentials and model names, then pass both into `AgentConfig`:
...
result = agent.research("What changed in EU AI Act enforcement this quarter?")
print(result.answer)
```

The same wiring works for any provider pair in the matrix; only the imports and constructor arguments change. In real applications, read secrets from environment variables or a secret manager rather than hard-coding them.
...
class LLMProvider(ABC):
    def complete(self, prompt, *, system=None, model=None,
                 timeout=600.0, state=None, deadline=None) -> str: ...
...
provider.models.effective_claim_extract_model   # falls back to reasoning
provider.models.effective_evaluate_model    # falls back to reasoning
provider.models.reasoning_model
```

The `resolve_claim_extract_model(llm, fallback)` helper in `inqtrix.strategies` is the canonical way to read the claim-extraction model; it emits a warning marker if neither constructor path nor reasoning fallback yields a value. Use that helper when you write custom strategy wiring so provider-specific model names do not leak back to global `Settings` defaults.
...
| Class | `search_model` |
|-------|----------------|
| `PerplexitySearch` | the configured model id, or the preset (default `fast-search`) |
