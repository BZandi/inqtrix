...
**Sparse vector (BM25)** — The lexical retrieval branch, stored as a vector but computed by an algorithm rather than a neural model: the Qdrant server itself (`Qdrant/bm25` core inference, server >= 1.15.3) tokenizes the text and emits one weight per occurring token and zero everywhere else, with the rarity (IDF) term likewise applied server-side. It is language-bound (`bm25_german` today), so cross-language keyword matching is structurally impossible except for shared tokens such as names and codes. See [Knowledge retrieval](../architecture/knowledge-retrieval.md#why-two-searches-instead-of-one).
