...
| Over-Searching | *Over-Searching in Search-Augmented Large Language Models* | Search-cost and abstention trade-offs | `compute_utility()` with marginal-gain formula; plateau detection |
...
- **Aspect coverage** as a formal completeness signal that caps confidence and drives targeted later-round queries.
...
- **Multiple stopping signals** (nine-step heuristic cascade, followed by a global `min_rounds` floor) rather than a single confidence threshold, including utility suppression for policy-critical questions.
