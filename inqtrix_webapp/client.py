...
def get_base_url() -> str | None:
    """Return the optional Inqtrix server base URL, without trailing slash."""
    raw = os.environ.get("INQTRIX_WEBAPP_BASE_URL", "").strip()
    if not raw:
        return None
    return raw.rstrip("/")
...
def _resolve_base_url(base_url: str | None) -> str | None:
    """Resolve an explicit URL first, then env URL, else ``None``."""
...
def fetch_health(base_url: str | None = None) -> dict[str, Any]:
    ...
    if not resolved_base_url:
        return {"status": "unreachable", "error": "Server URL not configured", "http_status": 0}
...
def fetch_stacks(base_url: str | None = None) -> dict[str, Any] | None:
    ...
    if not resolved_base_url:
        return None
    url = f"{resolved_base_url}/v1/stacks"
    try:
        with httpx.Client(timeout=_DEFAULT_SHORT_TIMEOUT) as client:
            resp = client.get(url)
...
def call_chat(
    ...
    *,
    ...
    resolved_base_url = _resolve_base_url(base_url)
    if not resolved_base_url:
        return {"error": {"message": "Server URL ist nicht konfiguriert."}}
...
def stream_chat(
    ...
    * ``("done", "")``           — clean terminator (not strictly needed by
    ...
    resolved_base_url = _resolve_base_url(base_url)
    if not resolved_base_url:
        yield ("error", "Server-URL ist nicht konfiguriert.")
        return
    url = f"{resolved_base_url}/v1/chat/completions"
    body = _build_chat_body(
        messages,
