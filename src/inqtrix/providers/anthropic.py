...
class AnthropicLLM(ThinkingSuppressionMixin, _NonFatalNoticeMixin, LLMProvider):
    ...
    def complete_with_metadata(
        ...
        *,
        ...
        # Send effort whenever the per-call model accepts it. Anthropic's
        # Messages API expects it inside output_config, not as a top-level
        # field (top-level returns HTTP 400 "Extra inputs are not permitted").
        # Models on the blacklist (Haiku-class) reject output_config.effort
        # → we skip the field for those calls but keep it for sibling calls
        # whose model does support it.
    ...
    def summarize_parallel(
        ...
        Args:
            ...
            deadline: Optional absolute monotonic deadline for the full
                agent run. Used to clamp the per-call timeout.
        ...
        Returns:
            ...
            first ``options.fallback_char_limit`` characters of the raw
            input (default 800) and both token counts are ``0``.
        ...
        payload: dict[str, Any] = {
            ...
            "max_tokens": preamble.max_output_tokens,
            "messages": [{"role": "user", "content": f"{preamble.prompt_template}{preamble.truncated_text}"}],
        }
        if self._temperature is not None:
            payload["temperature"] = self._temperature
