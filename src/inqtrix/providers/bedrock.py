...
class BedrockLLM(ThinkingSuppressionMixin, _NonFatalNoticeMixin, LLMProvider):
    ...
    def consume_effort_config_warnings(self) -> list[str]:
        ...
        Side effects:
            ...
            after rotating in a Haiku-class summarize model).
    ...
    def complete_with_metadata(
        ...
        *,
        ...
        # (which carries `effort`) live in the same dict.
        # See https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_Converse.html
        #
        # Effort is sent whenever the per-call model accepts it. Models on
        # the Haiku-class blacklist would 400-fail with output_config.effort,
        # so we omit it for those calls but keep it for sibling calls whose
        # model supports it. The without_thinking block (helper threads)
        # cannot raise this attribute either.
        additional_fields: dict[str, Any] = {}
        if use_thinking:
            additional_fields["thinking"] = self._thinking
        if self._effort_enabled() and _model_supports_effort(use_model):
            additional_fields["output_config"] = {"effort": self._effort}
    ...
    def summarize_parallel(
        ...
        Args:
            ...
            options: Optional :class:`SummarizeOptions` carrying tuning
                ...
                and the constructor-level token budget. This is the
        ...
        params: dict[str, Any] = {
            ...
            "messages": [{"role": "user", "content": [{"text": f"{preamble.prompt_template}{preamble.truncated_text}"}]}],
            "inferenceConfig": {"maxTokens": preamble.max_output_tokens},
        }
        if self._temperature is not None:
            params["inferenceConfig"]["temperature"] = self._temperature
