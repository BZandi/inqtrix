...
class PerplexitySearch(_NonFatalNoticeMixin, SearchProvider):
    ...
    def __init__(
        ...
        *,
        ...
        self._direct_mode = (
            ...
            else self._is_direct_perplexity_url(base_url)
        )

    @staticmethod
    def _is_direct_perplexity_url(base_url: str) -> bool:
        """Detect whether ``base_url`` points at the direct Perplexity API.

        Strict scheme+host match (``https://api.perplexity.ai``) so that proxies
        with the ``perplexity.ai`` substring in their hostname (e.g. an
        on-prem ``perplexity.ai-proxy.firma.de``) are not misclassified as
        the direct API path.
        """
        candidate = (base_url or "").strip().lower().rstrip("/")
        return (
            candidate == "https://api.perplexity.ai"
            or candidate.startswith("https://api.perplexity.ai/")
        )
    ...
    def is_available(self) -> bool:
        ...
        Configuration here means: the OpenAI SDK client was
        successfully constructed (the constructor enforces a non-
        empty ``api_key`` and a non-empty ``base_url``). Backend
        reachability or key validity is not pre-validated; invalid
        keys surface as upstream HTTP 401 on the first ``search()``
        call.
    ...
    def search_model(self) -> str:
        """Constructor-supplied Perplexity model identifier (e.g. ``"sonar-pro"``).

        Surfaces in ``GET /health`` and ``GET /v1/stacks`` so operators
        see the exact model the provider routes through. Mirrors the
        Constructor-First contract that ``LLMProvider.models`` follows.
        """
        return self._model
