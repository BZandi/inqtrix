...
def log_run_start(
    *,
    ...
    log.info(
        "RUN start: mode=%s llm=%s reasoning=%s classify=%s evaluate=%s summarize=%s default_max_tokens=%s search=%s engine=%s max_rounds=%d confidence_stop=%d max_total_seconds=%d testing_mode=%s question_len=%d history_len=%d seeded_session=%s",
        run_mode,
