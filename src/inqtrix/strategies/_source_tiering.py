...
class SourceTieringStrategy(ABC):
    ...
    def explain_url(self, url: str) -> dict[str, str]:
        """Explain how the strategy classified one URL.

        Custom strategies do not have to override this method. The
        default keeps the interface backwards-compatible by deriving a
        minimal explanation from :meth:`tier_for_url`.

        Args:
            url: Citation URL to classify.

        Returns:
            Mapping with ``domain``, ``tier``, and ``tier_reason`` keys.
        """
        tier = self.tier_for_url(url)
        return {
            "domain": domain_from_url(url),
            "tier": tier,
            "tier_reason": "strategy_default",
        }
...
class DefaultSourceTiering(SourceTieringStrategy):
    ...
    def explain_url(self, url: str) -> dict[str, str]:
        """Explain the domain-table match used for one URL."""
        domain = domain_from_url(url)
        if not domain:
            return {
                "domain": "",
                "tier": "unknown",
                "tier_reason": "no_domain",
            }
        ...
        if domain_matches(domain, low_domains):
            return {
                "domain": domain,
                "tier": "low",
                "tier_reason": "matched_low_domain",
            }
        if domain_matches(domain, PRIMARY_REGULATOR_DOMAINS):
            return {
                "domain": domain,
                "tier": "primary",
                "tier_reason": "matched_regulator_domain",
            }
        if domain_matches(domain, PRIMARY_OFFICIAL_COMPANY_DOMAINS):
            return {
                "domain": domain,
                "tier": "primary",
                "tier_reason": "matched_official_company_domain",
            }
        if domain_matches(domain, PRIMARY_ACADEMIC_INSTITUTION_DOMAINS):
            return {
                "domain": domain,
                "tier": "primary",
                "tier_reason": "matched_academic_institution_domain",
            }
        if domain_matches(domain, PRIMARY_SOURCE_DOMAINS):
            return {
                "domain": domain,
        ...
        if domain_matches(domain, MAINSTREAM_SOURCE_DOMAINS):
            return {
                "domain": domain,
                "tier": "mainstream",
                "tier_reason": "matched_mainstream_domain",
            }
        if domain_matches(domain, STAKEHOLDER_SOURCE_DOMAINS):
            return {
                "domain": domain,
                "tier": "stakeholder",
                "tier_reason": "matched_stakeholder_domain",
            }
        return {
            "domain": domain,
            "tier": "unknown",
            "tier_reason": "no_domain_table_match",
        }
