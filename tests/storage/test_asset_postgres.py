...
async def _section(store, sid, *, owner="u", created_at=1.0):
...
async def _asset(store, aid, *, owner="u", section_id="fsec_1", group_id=None,
    ...
    return await store.upsert_asset(
        ...
        created_at=created_at, updated_at=created_at, created_by_sub=owner,
        workspace_id=None,
    )
...
async def test_asset_list_excludes_body_get_includes_it(store) -> None:
    ...
    page, _ = await store.list_assets_page(created_by_sub="u", workspace_id=None, limit=50, after=None)
...
async def test_asset_keyset_walk_and_owner_scope(store) -> None:
    ...
    for n, ts in enumerate([10.0, 10.0, 20.0, 30.0, 30.0]):
        await _asset(store, f"fa_{n}", owner="u1", created_at=ts)
    await _asset(store, "fa_other", owner="u2", created_at=5.0)
    seen, cursor = [], None
    for _ in range(10):
        pg, nxt = await store.list_assets_page(created_by_sub="u1", workspace_id=None, limit=2, after=cursor)
...
async def test_asset_upsert_preserves_created_at_and_owner(store) -> None:
    await _section(store, "fsec_1")
    await _asset(store, "fa_1", owner="u1", created_at=100.0, text="v1")
    ...
    assert asset.created_by_sub == "u1"
