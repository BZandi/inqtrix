...
async def _save(service, sub, *, theme="dark", locale="de", contrast="high",
                preset="slate", updated_at=1.0):
    return await service.save_preferences(
        sub=sub, contrast_mode=contrast, locale=locale, theme=theme,
        theme_preset=preset, updated_at=updated_at,
    )
...
async def test_save_then_get_roundtrip(service) -> None:
    await _save(service, "u", theme="dark", locale="de", contrast="high", preset="sage")
...
async def test_save_is_whole_row_upsert(service) -> None:
    await _save(service, "u", theme="dark", preset="slate", updated_at=1.0)
    await _save(service, "u", theme="light", preset="graphite", updated_at=2.0)
    prefs = await service.get_preferences(sub="u")
