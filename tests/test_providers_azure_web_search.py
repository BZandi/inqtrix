...
def test_entra_id_passes_token_string_not_callable(mock_foundry_web):
    """Entra ID path must pass a plain token string, not a callable."""
    Cls, _ = mock_foundry_web
    with patch("inqtrix.providers.azure_web_search.OpenAI") as mock_openai:
        mock_openai.return_value = MagicMock()
        Cls(
...
def test_rate_limit_exception_re_raised(mock_foundry_web):
    """RateLimitError from the SDK is escalated to AgentRateLimited."""
    Cls, mock_client = mock_foundry_web

    request = httpx.Request("POST", "https://test.ai.azure.com/openai/v1/responses")
    response = httpx.Response(status_code=429, request=request, json={"error": {"message": "Rate limit exceeded"}})
    rate_error = RateLimitError(
        "rate limit",
        response=response,
        body={"error": {"message": "Rate limit exceeded"}},
    )
    mock_client.responses.create.side_effect = rate_error
...
def test_status_429_escalated_to_rate_limited(mock_foundry_web):
    """APIStatusError with status_code=429 is also escalated."""
    Cls, mock_client = mock_foundry_web

    request = httpx.Request("POST", "https://test.ai.azure.com/openai/v1/responses")
    response = httpx.Response(status_code=429, request=request, json={"error": {"message": "Too Many Requests"}})
    status_error = APIStatusError(
        "too many requests",
        response=response,
        body={"error": {"message": "Too Many Requests"}},
    )
    mock_client.responses.create.side_effect = status_error
...
def test_timeout_exception_re_raised(mock_foundry_web):
    """OpenAIError whose message mentions 'timed out' becomes AgentTimeout."""
    Cls, mock_client = mock_foundry_web
    mock_client.responses.create.side_effect = OpenAIError("Request timed out")
