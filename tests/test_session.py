...
class TestSessionStore:
    ...
    def test_save_truncates_answer_to_configured_limit(self):
        store = SessionStore(max_answer_chars=12)

        store.save("sid1", {}, "question", "0123456789abcdef")

        snap = store.get("sid1")
        assert snap is not None
        assert snap.last_answer == "0123456789ab"
...
class TestSessionIdDerivation:
    ...
    def test_stack_name_changes_session_id_for_same_conversation(self):
        """Multi-stack: same conversation against two stacks → two session ids."""
        conv = [
            {"role": "user", "content": "Frage"},
            {"role": "assistant", "content": "Antwort"},
            {"role": "user", "content": "Follow-up"},
        ]
        sid_default = derive_session_id(conv)
        sid_bedrock = derive_session_id(conv, stack_name="bedrock_perplexity")
        sid_anthropic = derive_session_id(conv, stack_name="anthropic_perplexity")

        assert sid_default is not None
        assert sid_bedrock is not None
        assert sid_anthropic is not None
        # Default (empty stack) and any named stack must differ.
        assert sid_default != sid_bedrock
        # Two different stacks must produce two different session ids.
        assert sid_bedrock != sid_anthropic

    def test_prospective_matches_derive_with_stack_name(self):
        msgs_turn1 = [{"role": "user", "content": "q1"}]
        pid = prospective_session_id(msgs_turn1, "a1", stack_name="bedrock_perplexity")
        msgs_turn2 = [
            {"role": "user", "content": "q1"},
            {"role": "assistant", "content": "a1"},
            {"role": "user", "content": "q2"},
        ]
        did = derive_session_id(msgs_turn2, stack_name="bedrock_perplexity")
        assert pid == did
